...
class User
{
    ...
    static public function find($id){
        $conn = Database::getConnection();
        $stmt = $conn->prepare("SELECT * FROM usuario WHERE id_usuario = :id_usuario"); //os : pratica que determina espaço resenvado
        $stmt->execute(['id_usuario' => $id]); // Executa o select
    ...
    static public function create($data){
        $conn = Database::getConnection();
        $stmt = $conn->prepare("INSERT INTO usuario (username, email, nome, sobrenome, senha, perfil, dataNasc)  VALUES (:username, :email, :nome, :sobrenome, :senha, :perfil, :dataNasc)"); // VALUES quais valores vão entrar na tabela
        $stmt->execute($data);
    }
